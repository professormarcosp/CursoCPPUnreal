	...
	if (SetaDaArma)
	{
		...
		if (AcertouEmAlgo)
		{
			...
			if (Ator->GetClass()->IsChildOf(ACharacter::StaticClass()) && ImpactoSangue)
			{
				...
				//Aqui Tentamos fazer um cast para o objeto do tipo do Player
				//APersonagemTPS se O ponteiro Jogador não tiver vazio
				//Significa que o cast deu certo e que o trace do tiro atingiu colidiu com um jogador
				APersonagemTPS* Jogador = Cast<APersonagemTPS>(Ator);
				//Então vai entrar neste if e setar o health dele menos 0.25 a cada tiro recebido
				if (Jogador != nullptr)
				{
					Jogador->SetHealth(10.f);
				} 
				else 
					//Se não for um jogador o cast irá falhar e o ponteiro Jogador
					//será nulo, e o código entrará aqui no else
				{
					//Tentamos novamente o cast mas desta vez para o tipo da classe pertencente ao objeto inimigo
					// Logo de o resultado da colisão do line trace que está em Ator
					//Puder ser realizado este Cast, significa que o tiro atingiu
					//um  inimigo e o Ponteiro Inimigo não será nulo
					ABotCharacter* Inimigo = Cast<ABotCharacter>(Ator);
					if (Inimigo != nullptr)
					{
						//Entrará neste if e iremos tirar 5 do health do Inimigo
