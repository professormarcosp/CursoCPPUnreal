...
void ABotAIController::OnSeePawn(APawn* SensedPawn)
{
	...
	//On seePawn recebe o objeto detectado no sensor do inimigo(botCharacter)
	//Mas SensedPawn retorna objeto da classe genérica do player (pawn)
	//Vamos fazer um CAST para a classe de nosso Player e colocar o resultado
	//deste objeto especializado em Player
	APersonagemTPS* Player = Cast<APersonagemTPS>(SensedPawn);
	//Somente de O inimigo tiver Health ele vai continuar a atirar e 
	//perseguir o player

	//Se inimigo estiver morto OU Player estiver morto
	//Coloque enemy como NULO para parar o Bot de perseguir ele, pois na BehaviorTree não estár SETADO
	// SET se for nullptr
	...
	//Se o health do bot fot maior que zero e o player NÂO(!) estive morto continue e persiga e atire no player
	if (Bot->GetHealth() > 0.0f && !Player->IsEstaMorto())
	{
